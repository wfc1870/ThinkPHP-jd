...
class GoodsModel extends Model{
	...
	/*******************/
		...
		//$data  这里虽然是多相册上传 但是是一条数据，不要弄错了 goods_id就只有一条
		...
		if($flag === true){
			...
			//这个只是附件的信息，不是这次插入操作的全部信息
			//所以遍历获取good_id不是这里的$v..这个只是附件的信息
			foreach($z as $k => $v){
				...
				//goods_id（$data['goods_id']）, pics_big, pics_small  这里的字段要和数据表里面的字段对应
				$arr = array(
						'goods_id' => $data['goods_id'],
				...
				//实现相册存储
				/***GoodsPics指的是_goods_pics表，你可能会说没有这个模型，可以这样用 不要下划线和大写首字母
				 * 你怎么只有哪个字母要大写根据下划线
				 * 
				 * 
				 * 
